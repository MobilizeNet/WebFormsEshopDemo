...
{
    public partial class CatalogItemHiLoGenerator : Mobilize.WebMap.Common.Core.IObservable, Mobilize.WebMap.Common.Core.IDelta
    {
#line hidden
        ...
        {
            refs["sequenceLock"] = _Mobilize_References[0];
            return refs;
        }
        ...
        {
            var ref_sequenceLock = Mobilize.WebMap.Common.Core.Context.CoreContext.Services.Instrumentation.GetAsObservable(this.sequenceLock);
            if (ref_sequenceLock != null)
                yield return ref_sequenceLock;
            yield break;
        }
        ...
        {
            if ((_Mobilize_DirtyFlag_0 & _Mobilize_Dirty_sequenceId) > 0)
            {
                yield return "sequenceId";
            }

            if ((_Mobilize_DirtyFlag_0 & _Mobilize_Dirty_remainningLoIds) > 0)
            {
                yield return "remainningLoIds";
            }

            if ((_Mobilize_DirtyFlag_0 & _Mobilize_Dirty_sequenceLock) > 0)
            {
                yield return "sequenceLock";
            }
        ...
        protected virtual bool _Mobilize_HasChanges(string propertyName)
        {
            ...
            {
                case "sequenceId":
                    return (_Mobilize_DirtyFlag_0 & _Mobilize_Dirty_sequenceId) > 0;
                case "remainningLoIds":
                    return (_Mobilize_DirtyFlag_0 & _Mobilize_Dirty_remainningLoIds) > 0;
                case "sequenceLock":
                    return (_Mobilize_DirtyFlag_0 & _Mobilize_Dirty_sequenceLock) > 0;
            }

            return false;
        }

        public CatalogItemHiLoGenerator()
        {
        ...
        const uint _Mobilize_Dirty_sequenceId = (uint)1 << 0;
        const uint _Mobilize_Dirty_remainningLoIds = (uint)1 << 1;
        const uint _Mobilize_Dirty_sequenceLock = (uint)1 << 2;
        const uint _Mobilize_Loaded_sequenceLock = (uint)1 << 0;
        private void Mobilize_Initialize_Properties()
        {
            this.sequenceId = -1;
#line hidden
            this.remainningLoIds = 0;
#line hidden
